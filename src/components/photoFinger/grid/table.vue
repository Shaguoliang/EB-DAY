<template>
    <div class="box">
      <ul v-for="(liData, rowindex) in ulData" :key="rowindex">
        <li v-for="(item, index) in liData" :key="index" @click="liClick(rowindex, index)">
          <div v-if="item.imgUrl.length != 0" :key="index" :class="[ 'radiusImg' , { imgBoard : item.select }]"  @click="chooseImg($event, index, item.imgUrl)">
